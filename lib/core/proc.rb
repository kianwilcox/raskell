...
class Proc
  ...
  def call(*args)
    args_to_consume = args.take(self.arity < 1 ? self.arity.abs-1 : self.arity) ## the < 1 here is because ruby stores *args as an arity of one more than the required args, in a negative number
    remaining_args = args.drop(self.arity < 1 ? self.arity.abs-1 : self.arity)
    
    if !args_to_consume.respond_to?(:length) || (self.arity < 0 && args.length >= self.arity.abs - 1) ## then we're dealing with a *args-based lambda, and we have enough args. just apply them all
      ...
      return self
    elsif args_to_consume.length < (self.arity < 1 ? self.arity.abs - 1 : self.arity)
